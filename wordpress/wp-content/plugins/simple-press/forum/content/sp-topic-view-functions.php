...
$LastChangedDate: 2013-02-09 04:13:21 -0700 (Sat, 09 Feb 2013) $
$Rev: 9832 $
*/
...
function sp_PostNewButton($args='', $label='', $toolTip='', $toolTipLock='') {
	...
	$out = '';
    $lock = false;
	if ($sfglobals['lockdown'] || $spThisTopic->forum_status || $spThisTopic->topic_status) {
		if(!empty($iconLock)) {
...
function sp_PostNewTopicButton($args='', $label='', $toolTip='', $toolTipLock='') {
	...
	$out = '';
    $lock = false;
	if ($sfglobals['lockdown'] || $spThisTopic->forum_status) {
		if(!empty($iconLock)) {
			$iconLock = SPTHEMEICONSURL.sanitize_file_name($iconLock);
...
function sp_PostIndexPageLinks($args='', $label='', $toolTip='') {
	...
	$out.= $label;
	$out.= sp_page_prev($sfvars['page'], $showLinks, $spThisTopic->topic_permalink, $pageLinkClass, $iconClass, $prevIcon, $nextIcon, $toolTip, '');
	...
	$out.= sp_page_next($sfvars['page'], $spThisTopic->total_pages, $showLinks, $spThisTopic->topic_permalink, $pageLinkClass, $iconClass, $prevIcon, $nextIcon, $toolTip, '');
	$out.= "</div>\n";
...
function sp_PostIndexPrint($args='', $label='', $toolTip='') {
	...
	$out = "<a class='$tagClass vtip' id='$tagId' title='$toolTip' rel='nofollow' href='javascript:void(null)' ";
	$out.= "onclick='jQuery(\"#spPostIndexContent".$spThisPost->post_id."\").jqprint({debug:true}); return false;'>";
...
function sp_PostIndexQuote($args='', $label='', $toolTip='') {
	global $spThisUser, $spThisPost, $spThisPostUser, $spThisTopic, $sfglobals;

	if ($spThisTopic->editmode) return;
	if ($spThisPost->post_status != 0 && !$spThisUser->admin) return;
	if (!sp_get_auth('reply_topics', $spThisTopic->forum_id)) return;
	if (($sfglobals['lockdown'] || $spThisTopic->forum_status || $spThisTopic->topic_status) && !$spThisUser->admin) return;
...
function sp_PostIndexEdit($args='', $label='', $toolTip='') {
	global $spThisPost, $spThisPostUser, $spThisTopic, $spThisUser, $sfglobals;

	if ($spThisTopic->editmode) return;
	if ($sfglobals['lockdown']) return;
...
function sp_PostIndexDelete($args='', $label='', $toolTip='') {
	global $spThisPost, $spThisTopic, $spThisUser, $sfglobals;
	...
	$out = "<form class='spButtonForm' action='$spThisPost->post_permalink' method='post' name='userdeletepost$spThisPost->post_id'>\n";
	$out.= '<input type="hidden" name="killpost" value="'.$spThisPost->post_id.'" />';
	$out.= '<input type="hidden" name="killposttopic" value="'.$spThisTopic->topic_id.'" />';
	$out.= '<input type="hidden" name="killpostforum" value="'.$spThisTopic->forum_id.'" />';
	$out.= '<input type="hidden" name="killpostposter" value="'.$spThisPost->user_id.'" />';
//$out.= '<a href="javascript: if(confirm(\''.$msg.'\')) {document.postkill'.$post['post_id'].'.submit();}">'.sp_text('Delete this post').'</a>';
	...
	$out.= "<a class='$tagClass vtip' id='$tagId' title='$toolTip' rel='nofollow' href='javascript: if(confirm(\"".$msg."\")) {document.userdeletepost$spThisPost->post_id.submit();}'>";
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt=''/>\n";
	if (!empty($label)) $out.= sp_filter_title_display($label);
	$out.= "</a>\n";
	$out.= '</form>'."\n";
...
function sp_PostForumToolButton($args='', $label='', $toolTip='') {
	global $spThisTopic, $spThisPost, $spThisPostUser, $spThisUser, $spGuestCookie, $sfglobals;
...
function sp_PostEditorWindow($addPostForm, $editPostForm) {
	global $spThisTopic, $spThisPost, $spThisUser, $sfglobals;
	...
	if ($spThisTopic->editmode) {
		...
		# New post form
		if ((sp_get_auth('reply_topics', $spThisTopic->forum_id)) && (!$spThisTopic->topic_status) && (!$sfglobals['lockdown']) || $spThisUser->admin) {
			$out = '<a id="dataform"></a>'."\n";
