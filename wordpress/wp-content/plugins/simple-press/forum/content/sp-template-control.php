...
Simple:Press
Template handler
$LastChangedDate: 2012-12-28 16:32:08 -0700 (Fri, 28 Dec 2012) $
$Rev: 9606 $
*/
...
# Uses the $sfvars 'pageview' to determine which template to load.
# Templates are always surrounded by the spMainContainer div
#
# --------------------------------------------------------------------------------------
function sp_process_template() {
	global $sfvars, $sfglobals, $spThisUser, $spNewPosts;
...
function sp_process_topic_view() {
	global $spTopicView, $sfvars, $spThisUser;

	if (!empty($sfvars['topicid'])) {
		# reduce unread counts for topic views if needed
		sp_remove_users_newposts($sfvars['topicid'], $spThisUser->ID);
	}
...
function sp_load_template($tempName) {
    ...
    $sfglobals, $sfvars, $SFDEVICE, $SFMOBILE;
...
function __sp($text) {
	global $sfglobals;
	return __($text, $sfglobals['themedomain']);
}
