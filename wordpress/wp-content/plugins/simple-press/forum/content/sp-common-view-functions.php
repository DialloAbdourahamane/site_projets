...
$LastChangedDate: 2013-02-22 08:05:20 -0700 (Fri, 22 Feb 2013) $
$Rev: 9870 $
*/
	...
	switch ($sectionName) {
		...
		case 'post':
            ...
            if (isset($spThisPost)) {
                ...
                if ($spThisPost->user_id == $spThisUser->ID) $rowClass.= ' spCurUserPost';
...
function sp_QuickLinksTopicMobile($args='', $label='') {
	...
	$defs = array('tagId'		=> 'spQuickLinksTopicMobile',
				  'tagClass'	=> 'spControl',
				  'listId'		=> 'spQuickLinksList',
				  'listClass'	=> 'spQuickLinksGroup',
	...
	if (!empty($spList->listData)) {
		$out.= "<div class='$tagClass' id='$tagId'>\n";
		$out.= "<p id='spQLTitle' onclick='spjOpenQL(\"$listId\", \"spQLOpener\", \"$openIcon\", \"$closeIcon\");'>$label<span id='spQLOpener'><img src='$openIcon' /></span></p>\n";
		$out.= "<div id='$listId' style='display:none'>\n";
		...
		foreach ($spList->listData as $spPost) {
			if ($spPost->forum_id != $thisForum) {
				if ($group) $out.= '</div>';
				$out.= "<div class='$listClass'><p>".sp_create_name_extract($spPost->forum_name, $length)."</p>\n";
				$thisForum = $spPost->forum_id;
				$group = true;
			}
			$class = 'spPostRead';
...
function sp_BreadCrumbs($args='', $homeLabel='') {
	...
	# profile link
	if (!empty($sfvars['profile'])) {
		$breadCrumbs.= $crumbEnd.str_repeat($crumbSpace, $treeCount)."<a class='$linkClass' href='".sp_url('profile')."'>".$icon.sp_text('Profile').'</a>';
	}

	# profile link
	if (!empty($sfvars['members']) && $sfvars['members'] == 'list') {
		$breadCrumbs.= $crumbEnd.str_repeat($crumbSpace, $treeCount)."<a class='$linkClass' href='".sp_url('members')."'>".$icon.sp_text('Members List').'</a>';
	}
...
function sp_BreadCrumbsMobile($args='', $forumLabel='') {
	...
	if (!empty($sfvars['forumslug']) && $sfvars['forumslug'] != 'all') {
		# if showing a topic then check the return page of forum in transient store
		$returnPage = (empty($sfvars['topicslug'])) ? 1 : sp_pop_topic_page($sfvars['forumid']);
		$breadCrumbs.= "<a class='$tagClass' href='".sp_build_url($sfvars['forumslug'], '', $returnPage, 0)."'>".sp_truncate(sp_filter_title_display($sfvars['forumname']), $truncate)."</a>\n";
	}
...
function sp_UnreadPostsInfo($args='', $label='', $unreadToolTip='', $markToolTip='', $popupLabel='') {
	...
	if($mobileMenu) {
		# Run as page not a popup by default
		if ($unreads > 0) {
			$out.= "<li class='$tagClass' id='$tagId'><a rel='nofollow' id='$unreadLinkId' href='".sp_url('newposts')."'>$label</a></li>\n";
			$out.= "<li class='$tagClass'><a href='".SFMARKREAD."'>$markToolTip</a></li>\n";
		}
	} else {
		...
		if ($unreads > 0) {
			if ($popup) {
				$site = SFHOMEURL."index.php?sp_ahah=newpostpopup&amp;action=all&amp;first=$first&amp;sfnonce=".wp_create_nonce('forum-ahah');
...
function sp_SearchForm($args='') {
	...
	$out = "<form id='$tagId' action='".SFHOMEURL."index.php?sp_ahah=search&amp;sfnonce=".wp_create_nonce('forum-ahah')."' method='post' name='sfsearch' onsubmit='return spjValidateSearch(\"form\", {$sfglobals['mysql']['search']['min']});'>";
	...
	$out.= "<a rel='nofollow' id='$submitId' class='$submitClass vtip' title='$toolTip' onclick='spjValidateSearch(this, \"$submitId\", \"link\", {$sfglobals['mysql']['search']['min']});'>";
	if(!empty($icon)) {
