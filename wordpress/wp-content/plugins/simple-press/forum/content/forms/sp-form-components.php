...
$LastChangedDate: 2013-02-17 14:00:58 -0700 (Sun, 17 Feb 2013) $
$Rev: 9860 $
*/
...
global $sfglobals;
$toolbar = $sfglobals['display']['editor']['toolbar'];
...
function sp_topic_editor_submit_buttons($out, $spThisForum, $a, $toolbar) {
	...
	foreach($cOrder as $c) {
		switch($c) {
			case 'save':
			if ($SFDEVICE == 'mobile' && array_key_exists('iconMobileSubmit', $a) && !empty($a['iconMobileSubmit'])) {
				# display mobile icon
			...
			if ($SFDEVICE == 'mobile' && array_key_exists('iconMobileCancel', $a) && !empty($a['iconMobileCancel'])) {
				# display mobile icon
...
function sp_topic_editor_default_buttons($out, $spThisForum, $a, $toolbar) {
	global $sfglobals, $spThisUser, $SFDEVICE;
	extract($a, EXTR_SKIP);
	...
	if ($display['smileys']) {
		if($SFDEVICE == 'mobile' && array_key_exists('iconMobileSmileys', $a) && !empty($a['iconMobileSmileys'])) {
			# display mobile icon
	...
	if ($display['options']) {
		if($SFDEVICE == 'mobile' && array_key_exists('iconMobileOptions', $a) && !empty($a['iconMobileOptions'])) {
			# display mobile icon
...
function sp_topic_editor_smileys_options($out, $spThisForum, $a, $toolbar) {
	...
	if ($display['options']) {
		...
		if ($toolbar=='toolbar') $optionsBox.= sp_InsertBreak('direction=both&spacer=6px&echo=0');
		$optionsBox.= '</div>'."\n";
	}
...
function sp_post_editor_default_buttons($out, $spThisTopic, $a, $toolbar) {
	global $sfglobals, $sfvars, $spThisUser, $SFDEVICE;
	extract($a, EXTR_SKIP);
