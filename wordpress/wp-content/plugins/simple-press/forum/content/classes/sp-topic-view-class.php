...
Simple:Press
Topic View Class
$LastChangedDate: 2012-12-18 13:23:37 -0700 (Tue, 18 Dec 2012) $
$Rev: 9538 $
*/
...
class spTopicView {
	...
	function __construct($id=0, $page=0) {
		global $sfvars;
		if (($id==0) && (!empty($sfvars['topicid']))) $id = $sfvars['topicid'];
		$this->topicId = $id;
		$this->parentForum = $sfvars['forumid'];
	...
	function sp_topicview_query($topicid=0, $cPage=1, $forumid=0) {
		...
		# Set up order by
        $reverse = (array_search($topicid, (array) $sfglobals['sort_order']['topic']) !== false) ? true : false;
		$ORDER = ($sfglobals['display']['posts']['sortdesc'] XOR $reverse) ? 'DESC' : 'ASC';
		$sort = $ORDER;
		...
		$spdb = new spdbComplex;
			$spdb->table 	= SFTOPICS;
			$spdb->fields	= SFTOPICS.'.topic_id, '.SFTOPICS.'.forum_id, topic_name, topic_slug, topic_status, topic_icon, topic_opened, '.SFTOPICS.'.post_count, forum_slug, forum_status, forum_rss_private,
