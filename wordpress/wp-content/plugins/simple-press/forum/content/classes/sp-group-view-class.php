...
class spGroupView {
	...
	function __construct($ids='', $stats=true) {
		...
		if (!empty($sfvars['singlegroupid']) && !in_array($sfvars['singlegroupid'], $gIds)) $gIds[] = $sfvars['singlegroupid'];
	...
	function sp_groupview_query($groupids='') {
		...
		$spdb = new spdbComplex;
			...
			$spdb->fields	= SFGROUPS.'.group_id, group_name, group_desc, group_rss, group_icon, group_message,
							forum_id, forum_name, forum_slug, forum_desc, forum_status, forum_icon, forum_icon_new, forum_rss_private,
