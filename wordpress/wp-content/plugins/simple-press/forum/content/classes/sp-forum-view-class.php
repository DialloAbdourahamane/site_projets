...
Simple:Press
Forum View Class
$LastChangedDate: 2013-01-17 18:20:30 -0700 (Thu, 17 Jan 2013) $
$Rev: 9738 $
*/
...
class spForumView {
	...
	function __construct($id=0, $page=0) {
		global $sfvars;
		if (($id==0) && (!empty($sfvars['forumid']))) $id = $sfvars['forumid'];
		$this->forumId = $id;

		if (($page==0) && (!empty($sfvars['page']))) $page = $sfvars['page'];
		$this->forumPage = $page;
	...
	#
	#	$forumid:	Can pass an id (or will pick up from $sfvars if available)
	...
	function sp_forumview_query($forumid=0, $cPage=1) {
        ...
        $reverse = (array_search($forumid, (array) $sfglobals['sort_order']['forum']) !== false) ? true : false;
		if ($sfglobals['display']['topics']['sortnewtop'] XOR $reverse) {
		...
		if ($records) {
			...
			if (sp_can_view($forumid, 'topic-title')) {
				...
				if ($f[$fidx]->children) {
					$subs = $this->sp_forumview_subforums_query(unserialize($f[$fidx]->children));
					if ($subs) {
			...
			if ($r) {
				...
				if (isset($f[$forumid]->children) && $f[$forumid]->children) {
					$subs = $this->sp_forumview_subforums_query(unserialize($f[$forumid]->children));
					if ($subs) {
		...
		if (!empty($p)) {
			...
			if ($stats) {
				foreach ($subs as $sub) {
					...
					$fsub = $f[$fidx]->subforums[$sub->forum_id]->forum_id_sub;
					...
					$f[$fidx]->subforums[$sub->forum_id] = apply_filters('sph_forumview_subforum_records', $f[$fidx]->subforums[$sub->forum_id], $sub);
				}
			}
			unset($subs);
			unset($stats);
	...
	# --------------------------------------------------------------------------------------
	#
	#	sp_forumview_subforums_query($subs)
	#	Builds sub-forum list for the ForumView template
	#
	#	$subs:	Array of the children/sub forum ids for the forum in forum view
	#
	# --------------------------------------------------------------------------------------
	function sp_forumview_subforums_query($subs) {
		...
		$spdb = new spdbComplex;
			$spdb->table	= SFFORUMS;
			$spdb->fields	= 'forum_id, forum_name, forum_slug, forum_desc, forum_status, forum_icon, topic_icon, topic_icon_new, forum_rss_private,
