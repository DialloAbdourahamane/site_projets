...
$LastChangedDate: 2013-02-08 11:10:17 -0700 (Fri, 08 Feb 2013) $
$Rev: 9824 $
*/
...
function sp_ProfileEdit($tabSlug='profile', $menuSlug='') {
	...
	global $sfvars, $sfglobals, $spThisUser;
...
function sp_ProfileShowSearchPosts($args='', $label='', $labelStarted='', $labelPosted='') {
	...
	if ($spProfileUser->ID == $spThisUser->ID) {
		...
		$text1 = sprintf(spa_text('List Topics %1$s Has Posted To'), $spProfileUser->display_name);
		$text2 = sprintf(spa_text('List Topics %1$s Has Started'), $spProfileUser->display_name);
	}
...
function sp_SetupSigEditor($content='') {
	...
	$out.= apply_filters('sph_editor_textarea', $out, 'postitem', $content, $sfglobals['editor'], '');
