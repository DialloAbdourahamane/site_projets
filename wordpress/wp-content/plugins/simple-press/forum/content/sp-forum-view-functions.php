...
function sp_SubForumIndexLockIcon($args='', $toolTip='') {
	global $spThisSubForum, $sfglobals, $spThisUser;
	...
	if ($sfglobals['lockdown'] || $spThisSubForum->forum_status)  {
...
function sp_SubForumIndexAddIcon($args='', $toolTip='') {
    ...
    # add new topic icon
	if (sp_get_auth('start_topics', $spThisSubForum->forum_id) && ((!$spThisSubForum->forum_status && !$sfglobals['lockdown']) || $spThisUser->admin)) {
		$url = sp_build_url($spThisSubForum->forum_slug, '', 1, 0).sp_add_get().'new=topic';
...
function sp_ForumHeaderSubForums($args='', $label='', $toolTip='') {
	...
	foreach ($spThisForumSubs as $sub) {
		if ($sub->top_parent == $sub->parent && $sub->forum_id != $spThisSubForum->forum_id) {
			# skip this one - it is a sibling to the main subforum not a child
			continue;
		}
...
function sp_TopicEditorWindow($addTopicForm) {
	global $spThisUser, $spThisForum, $sfglobals;

	if ((sp_get_auth('start_topics', $spThisForum->forum_id)) && (!$spThisForum->forum_status) && (!$sfglobals['lockdown']) || $spThisUser->admin) {
		$out = '<a id="dataform"></a>'."\n";
