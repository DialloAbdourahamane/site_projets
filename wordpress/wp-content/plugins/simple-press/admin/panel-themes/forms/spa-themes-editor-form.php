...
<?php
	...
	spa_paint_open_tab(spa_text('SP Theme Editor').' - '.spa_text('Edit Simple:Press Themes'));
	...
	if ($stylesheets_dir) {
		while (($subfile = readdir($stylesheets_dir)) !== false) {
			if (substr($subfile, 0, 1) == '.') continue;
			if (substr($subfile, -4) == '.php' || substr($subfile, -4) == '.css') $stylesheets[] = $subfile;
		}
	}
