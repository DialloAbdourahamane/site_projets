...
Simple:Press
Admin themes mobile
$LastChangedDate: 2012-11-18 18:58:11 -0700 (Sun, 18 Nov 2012) $
$Rev: 9316 $
*/
...
<?php
    if ($mobileTheme['active']) {
    	...
    	spa_paint_open_tab(spa_text('Available Themes')." - ".spa_text('Select Simple:Press Mobile Theme'));
    	spa_paint_open_panel();
    ...
    ?>
    				...
    				<?php } ?>
    					...
    					<span class="action-links">
    	                    <script type="text/javascript">
    	                    jQuery(document).ready(function() {
    	                    	jQuery('#sftheme-<?php echo esc_js($theme_file); ?>').ajaxForm({
    	                    		target: '#sfmsgspot',
    	                    		success: function() {
    	                                jQuery('#sfreloadmlist').click();
    	                    			jQuery('#sfmsgspot').fadeIn();
