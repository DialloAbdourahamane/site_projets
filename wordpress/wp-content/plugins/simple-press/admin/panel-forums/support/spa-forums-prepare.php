...
function spa_edit_forum_sequence_options($action, $type, $id, $current) {
	...
	if ($type == 'subforum') {
		$parent = spdb_table(SFFORUMS, "forum_id=$id", 'parent');

		$forum = spdb_table(SFFORUMS, "forum_id=$parent", 'row');
...
function spa_paint_custom_icons() {
	...
	$dlist = @opendir($path);
	if (!$dlist) {
		echo '<tr><td class="sflabel"><strong>'.spa_text('The custom icons folder does not exist').'</strong></td></tr>';
		return;
	}
