...
function spa_set_group_permission($group_id, $usergroup_id, $permission) {
	...
	if ($forums) {
		...
		$mess = spa_text('Group has no members so no permission sets were added');
	}
...
function spa_save_forums_delete_forum() {
	...
	if ($topics) {
		foreach ($topics as $topic) {
			sp_delete_topic($topic->topic_id, false);
		}
	}
