...
$LastChangedDate: 2013-01-01 11:27:18 -0700 (Tue, 01 Jan 2013) $
$Rev: 9652 $
*/
...
function spa_save_smileys_data() {
	...
	$mess = '';
	# save Smiley option -------------------
	sp_update_option('sfsmileys', isset($_POST['sfsmallow']));
...
function spa_save_login_data() {
	...
	if (!empty($_POST['sfloginemailurl']))
		...
		$sflogin['sfloginemailurl'] = SFHOMEURL.'wp-login.php?action=login&amp;redirect_to='.sp_url();
...
function spa_add_special_rank_member($id) {
	...
	for ($x=0; $x<count($user_id_list); $x++) {
        $memberData = sp_get_member_item((int) $user_id_list[$x], 'special_ranks');
        $memberData[] = $rank[0]['meta_key'];
        sp_update_member_item((int) $user_id_list[$x], 'special_ranks', $memberData);
	}
...
function spa_del_special_rank_member($id) {
	...
	for ($x=0; $x<count($user_id_list); $x++) {
        $memberData = sp_get_member_item((int) $user_id_list[$x], 'special_ranks');
        if (empty($memberData)) continue;
		$newList = array();
		foreach ($memberData as $id) {
			if ($id != $rank[0]['meta_key']) $newList[] = $id;
		}
        sp_update_member_item((int) $user_id_list[$x], 'special_ranks', $newList);
	}
