...
function spa_get_smileys_data() {
	$sfcomps = array();
	$sfcomps['sfsmallow'] = sp_get_option('sfsmileys');
...
function spa_paint_custom_smileys() {
	...
	$dlist = @opendir($path);
	if (!$dlist) {
	   echo '<tr><td class="sflabel"><strong>'.spa_text('The forum-smileys folder does not exist').'</strong></td></tr>';
       return;
    }
