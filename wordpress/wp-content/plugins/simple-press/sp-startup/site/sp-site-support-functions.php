...
$LastChangedDate: 2012-12-21 18:51:31 -0700 (Fri, 21 Dec 2012) $
$Rev: 9567 $
*/
	...
	if ($slug) {
		...
		if ($page) {
			...
			if (get_option('page_on_front') == $page->ID && get_option('show_on_front') == 'page') {
				...
				$newslug = trim(substr($perm, strlen(SFSITEURL)), '/');
				if($newslug != $slug) {
					sp_update_option('sfslug', $newslug);
				}
			}
...
function sp_mobile_check() {
	global $SFMOBILE, $SFDEVICE;

	$SFDEVICE = sp_detect_device();
	if ($SFDEVICE == 'mobile' || $SFDEVICE == 'tablet') {
		$SFMOBILE = true;
	}
//==>
//$SFMOBILE=true;
...
//==>
	...
	$SFDEVICE = apply_filters('sph_device_check', $SFDEVICE);
}
