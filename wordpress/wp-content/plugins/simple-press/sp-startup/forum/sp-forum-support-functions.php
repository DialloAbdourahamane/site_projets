...
Simple:Press
Desc:
$LastChangedDate: 2013-02-21 04:01:40 -0700 (Thu, 21 Feb 2013) $
$Rev: 9867 $
*/
...
function sp_populate_query_vars() {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		if (empty($sfvars['groupid'])) $sfvars['groupid'] = 0;
...
function sp_setup_page_type() {
	...
	if ($pageview == '') {
		if (!empty($sfvars['feed'])) {
			...
			if (isset($_GET['group'])) $sfvars['singlegroupid'] = sp_esc_int($_GET['group']);
...
function sp_get_metakeywords() {
    ...
    if (!empty($sfmetatags['sfusekeywords'])) {
        if ($sfmetatags['sfusekeywords'] == 3) {
            if (($sfvars['pageview'] == 'forum' || $sfvars['pageview'] == 'topic')) {
                $keywords = sp_filter_title_display(spdb_table(SFFORUMS, 'forum_id='.$sfvars['forumid'], 'keywords'));
            } else {
...
function sp_setup_title($title, $sep) {
	...
	$forumslug = (!empty($sfvars['forumslug'])) ? $sfvars['forumslug'] : '';
	$topicslug = (!empty($sfvars['topicslug'])) ? $sfvars['topicslug'] : '';
	...
	if ($sfseo['sfseo_page']) {
		$profile = (!empty($sfvars['profile'])) ? urlencode($sfvars['profile']) : '';
		...
		$list = (!empty($sfvars['members'])) ? $list = urlencode($sfvars['members']) : '';
...
function sp_load_editor($override = 0, $supportOnly = 0) {
	...
	if ($override != 0) $sfglobals['editor'] = $override;

    # allow plugins to control editor choice
    $sfglobals['editor'] = apply_filters('sph_this_editor', $sfglobals['editor'], $override, $supportOnly);
