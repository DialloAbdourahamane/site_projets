...
function sp_load_forum_scripts() {
	global $sfvars, $SFMOBILE, $SFDEVICE;
    ...
    $script = (defined('SP_SCRIPTS_DEBUG') && SP_SCRIPTS_DEBUG) ? SFJSCRIPT.'print/jquery.jqprint-dev.js' : SFJSCRIPT.'print/jquery.jqprint.js';
	sp_plugin_enqueue_script('sfprint', $script, array('jquery'), false, $footer);
    ...
    if ($combine_js) { # use compressed scripts
        ...
        if (!empty($sp_plugin_scripts)) {
            foreach ($sp_plugin_scripts->queue as $handle) {
                # enqueue with wp
                $plugin_footer = (is_array($sp_plugin_scripts->registered[$handle]->extra) && $sp_plugin_scripts->registered[$handle]->extra['group'] == 1) ? true : false;
...
#-----------------------------------------------
		...
		if (isset($_POST['approvepost'])) sp_approve_post(false, sp_esc_int($_POST['approvepost']), $sfvars['topicid']);
