...
$LastChangedDate: 2013-01-02 15:47:12 -0700 (Wed, 02 Jan 2013) $
$Rev: 9656 $
*/
...
function sp_send_email($mailto, $mailsubject, $mailtext, $replyto='') {
	global $sfglobals;
	$sfglobals['replyAddress'] = $replyto;
    $sfmail = array();
    $sfmail = sp_get_option('sfmail');
    if (($sfmail['sfmailuse']) || (!empty($replyto))) {
...
function sp_mail_filter_from($from) {
	global $sfglobals;
	$replyAddress = $sfglobals['replyAddress'];
...
function sp_splice($text, $pos=10, $method=0) {
	$label = array();
	$label = explode(' ', $text);
	switch($method) {
		case 0:
