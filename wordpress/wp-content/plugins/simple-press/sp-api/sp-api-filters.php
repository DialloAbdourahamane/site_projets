...
Simple:Press
Filters
$LastChangedDate: 2013-03-11 11:00:37 -0700 (Mon, 11 Mar 2013) $
$Rev: 10053 $
*/
...
function sp_filter_content_save($content, $action) {
	...
	if ($action == 'edit') {
		if (function_exists('sp_editor_prepare_edit_content')) $content = sp_editor_prepare_edit_content($content, $sfglobals['editor']);
	}

	# 3: convert code tags to our own code display tags and parse for inine bbCode
	$content = sp_filter_save_codetags1($content, $sfglobals['editor'], $action);
	...
	$content = sp_filter_save_codetags2($content, $sfglobals['editor'], $action);
    ...
    $fid = (isset($sfvars['forumid'])) ? $sfvars['forumid'] : '';
...
function sp_filter_content_edit($content) {
	...
	if (function_exists('sp_editor_save_linebreaks')) {
		$content = sp_editor_save_linebreaks($content, $sfglobals['editor']);
	} else {
		$content = sp_filter_save_linebreaks($content);
	}

	if (function_exists('sp_editor_format_paragraphs_edit')) $content = sp_editor_format_paragraphs_edit($content, $sfglobals['editor']);

	# 4: Parse post into appropriate editor format
	$content = sp_filter_edit_parser($content, $sfglobals['editor']);
...
function sp_filter_display_smileys($content) {
	global $sfglobals, $wp_smiliessearch;
	...
	switch ($style[1]) {
		...
		case 'margin: 0 auto':
			...
			if ($sfvars['postwidth'] > 0) {
				$p = (abs($sfvars['postwidth']/2) + abs($thumb/2)-40);
...
function sp_filter_rss_display($content) {
    ...
    # 6: hide links
    if (!sp_get_auth('view_links', $sfvars['forumid'])) $content = sp_filter_display_hidelinks($content);
