...
#==>
	$result = '';
# DO NOR REMOVE COMMENTED OUT SEGMENT
# This really needs all our DB stuf to be updated to use the new api - especially the
# spdbComplex class. When that cinversin is made this needs to be revisted.
#	if(($qtype == 'row' || $qtype == 'var') && $rettype==OBJECT && $ISFORUMADMIN==false) {
#		$result = sp_get_cache_datarow($table, $where, $qtype, $varcol);
#	}

	if(empty($result)) {
		$sql = "SELECT $selectfrom FROM $table$whereclause$orderby$limit";
		$result = spdb_select($qtype, $sql, $rettype);
# DO NOR REMOVE COMMENTED OUT SEGMENT YET
# see message above
#		if($result && $qtype=='row' && $rettype==OBJECT) {
#			sp_add_cache_datarow($table, $result);
#		}
	}
	return $result;
#==>
...
# Returns false if good but no records
# Returns false if failed and displays error if sql invalid
# Populates sfvars['queryrows'] with number of records
#
...
function spdb_select($querytype, $sql, $resulttype=OBJECT) {
	...
	if ($wpdb->last_error == '') {
		$sfvars['queryrows'] = $wpdb->num_rows;
	} else {
...
# Returns false if failed and displays error if sql invalid
# Populates sfvars['affectedrows'] with number of records
# If INSERT populates sfvars['insertid'] with auto number
#
#	$sql:		SQL query statement
# ------------------------------------------------------------------
function spdb_query($sql) {
	...
	if ($wpdb->last_error == '') {
		$sfvars['affectedrows'] = mysql_affected_rows();
		if(substr($sql, 0, 6) == 'INSERT') $sfvars['insertid'] = $wpdb->insert_id;
		return true;
	} else {
		sp_construct_database_error($sql, mysql_error());
		return false;
