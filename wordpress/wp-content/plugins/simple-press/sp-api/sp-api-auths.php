...
$LastChangedDate: 2013-03-11 11:27:49 -0700 (Mon, 11 Mar 2013) $
$Rev: 10054 $
*/
		...
		if ($forums) {
			foreach ($forums as $forum) {
				foreach ($sfglobals['auths_map'] as $auth) {
				    if ($sfglobals['auths'][$auth]->admin_negate) {
...
function sp_is_forum_admin($userid) {
	...
	if ($userid) {
		if (is_multisite() && is_super_admin($userid)) {
			...
			if(!isset($sfglobals['forum-admins']) || empty($sfglobals['forum-admins'])) {
				$sfglobals['forum-admins'] = sp_get_admins();
			}
			$is_admin = array_key_exists($userid, $sfglobals['forum-admins']);
		}
	}
	return $is_admin;
}
...
function sp_is_forum_mod($userid) {
	global $sfglobals;
	$is_mod = 0;
	if ($userid && isset($sfglobals['forum_moderators']) && !empty($sfglobals['forum_moderators'])) {
		foreach($sfglobals['forum_moderators'] as $x) {
			foreach($x as $y) {
				foreach($y as $z) {
...
function sp_add_auth($name, $desc, $active=1, $ignored=0, $enabling=0, $negate=0, $auth_cat=1) {
	...
	if (empty($auth)) {
		...
		# snure we get the right auth cat id in case users are ordered in a non-standard sequence
...
function sp_current_user_can($cap) {
	...
	$allow_wp_admins = (empty($sfglobals['forum-admins']) && is_super_admin()) ? true : false;
