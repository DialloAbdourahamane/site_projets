...
function sp_get_user_special_ranks($userid) {
	...
	$userRanks = array();
    $memberRanks = sp_get_member_item($userid, 'special_ranks');
    if (empty($sfglobals['special_rank']) || empty($memberRanks)) return $userRanks;

	$count = 0;
	foreach ($sfglobals['special_rank'] as $key => $rank) {
...
function sp_get_user_forum_rank($usertype, $userid, $userposts) {
	...
	$rankdata =  array();
	if ($usertype == 'User' && !empty($sfglobals['forum_rank'])) {
...
function sp_build_avatar_display($userid, $avatar, $link) {
	...
	switch ($link) {
		case 'profile':
			...
			$forumid = (!empty($sfvars['forumid'])) ? $sfvars['forumid'] : '';
...
function sp_attach_user_profile_link($userid, $targetitem) {
	...
	switch ($sfprofile['displaymode']) {
		...
		case 3:
			...
			$user = new WP_User($userid);
			$site = SFSITEURL.user_trailingslashit('members/'.str_replace(' ', '', urlencode($user->user_login)).'/profile');
...
function sp_build_profile_formlink($userid) {
	...
	switch ($sfprofile['formmode']) {
		...
		case 3:
			# BuddyPress profile page
			$user = new WP_User($userid);
			$site = SFSITEURL.'members/'.str_replace(' ', '', urlencode($user->user_login)).'/profile/edit/';
            $site = apply_filters('sph_buddypress_profile', $site, $user);
			return $site;
			break;
...
function sp_get_topic_url($forumslug, $topicslug, $topicname) {
	...
	if (isset($sfvars['searchvalue']) && $sfvars['searchvalue']) {
		...
		$out.= '='.$sfvars['searchvalue'].'&amp;type='.$sfvars['searchtype'].'&amp;include='.$sfvars['searchinclude'].'&amp;scope='.'&amp;search='.$sfvars['searchpage'].'">'.$topicname.'</a>'."\n";
	} else {
