<?php 
/**
 Template Name: Envoyer message a tous les etu seuls
 */
...
<form  name="mail_form" method="post" action="<?php echo get_stylesheet_directory_uri(); ?>/testEnvoiAllEtuSeul.php" target="target"> 
  	<fieldset class="style-fieldset" > 
   <legend class="style-legend"> Envoyer un message aux étudiants sans groupe </legend> 
...
$q="SELECT user_email FROM wp_users  
	WHERE ID not in (select id_membre from wp_id_group_membres )";
 
 $emails=$wpdb->get_results($q);
foreach( $users as $userEtu ) {
	foreach( $emails as $user ) {
	if($user->user_email === $userEtu->user_email){
